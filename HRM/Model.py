...
class HierarchicalReasoningModel_ACTV1_Inner(nn.Module):
    ...
    def _input_embeddings(self, input: torch.Tensor, puzzle_identifiers: torch.Tensor):
        ...
        # Puzzle embeddings
        if self.config.puzzle_emb_ndim > 0:
            # Ensure puzzle_identifiers matches batch size
            if puzzle_identifiers.numel() == 1:
                puzzle_identifiers = puzzle_identifiers.expand(batch_size)
            elif puzzle_identifiers.shape[0] != batch_size:
                puzzle_identifiers = puzzle_identifiers[:batch_size]  # Take first batch_size elements
    ...
    def reset_carry(self, reset_flag: torch.Tensor, carry: HierarchicalReasoningModel_ACTV1InnerCarry):
        ...
        return HierarchicalReasoningModel_ACTV1InnerCarry(
            z_H=torch.where(reset_flag.view(-1, 1, 1), self.H_init, carry_z_H),
            z_L=torch.where(reset_flag.view(-1, 1, 1), self.L_init, carry_z_L),
        )
...
class HierarchicalReasoningModel_ACTV1(nn.Module):
    ...
    def forward(self, carry: HierarchicalReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor]) -> Tuple[HierarchicalReasoningModel_ACTV1Carry, Dict[str, torch.Tensor]]:
        ...
        device = next(self.parameters()).device  # Get model's device
        carry_halted = carry.halted.to(device)

        new_current_data = {}
        for k, v in carry.current_data.items():
            batch_k = batch[k].to(device)
            v = v.to(device)
            mask_shape = (-1,) + (1,) * (batch_k.ndim - 1)
            new_current_data[k] = torch.where(carry_halted.view(mask_shape), batch_k, v)
...
if __name__ == "__main__":
    ...
    inputs = torch.randint(0, 1000, (16,128))
    puzzle_identifiers = torch.randint(0,2, (1,))
    batch = {"inputs":inputs, "puzzle_identifiers":puzzle_identifiers}
    carry = model.initial_carry(batch=batch)
    carry, out = model(batch=batch, carry=carry)
    print(out)
    
    print("\n\n")
    print("""Let \( a, b, c \) be positive real numbers. Prove that
